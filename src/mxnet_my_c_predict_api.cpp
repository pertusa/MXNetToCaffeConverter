...
int MXPredCreatePartialOut(const char* symbol_json_str,
  ...
  // bind
  {
    ...
    ret->grad_store= grad_store;
    ret->grad_req= grad_req;
    ret->arg_params=arg_params;
    ret->aux_params=aux_params;
...
///// ADDED METHODS (pertusa)

MXNET_DLL int MXGetArgParams(PredictorHandle handle,
                                std::string layer_name,
                                mx_float *data,
                                mx_uint   size) // Size is passed for checking
{
  MXAPIPredictor* p = static_cast<MXAPIPredictor*>(handle);
  API_BEGIN();

  CHECK_GT(p->arg_params.count(layer_name),0);

  // Calculate MXNet layer size and check that it is the same than the received size
  TShape shape=p->arg_params[layer_name].shape();
  unsigned mxnet_size=1;
  for (mx_uint i=0; i<shape.ndim(); i++)
    mxnet_size*=shape[i];
  CHECK_EQ(size,mxnet_size);

  // Copy to float*
  const NDArray& nd = p->arg_params[layer_name];
  nd.SyncCopyToCPU(data, mxnet_size);

  API_END();
}
     
MXNET_DLL int MXGetAuxParams(PredictorHandle handle,
                                std::string layer_name,
                                mx_float *data,
                                mx_uint size)
{
  ...
  CHECK_GT(p->aux_params.count(layer_name),0);
  
  // Calculate MXNet layer size and check that it is the same than the received size
  TShape shape=p->aux_params[layer_name].shape();
  unsigned mxnet_size=1;
  for (mx_uint i=0; i<shape.ndim(); i++) 
    mxnet_size*=shape[i];
  CHECK_EQ(size,mxnet_size);
  ...
  // Copy to float*
  const NDArray& nd = p->aux_params[layer_name];
  nd.SyncCopyToCPU(data, mxnet_size);
